...
/**
  * 普通函数类举例
  */
object FunctionClassTransformation2 {
  private[this]val logger =Logger(this.getClass)
  def main(args: Array[String]): Unit = { //初始化上下文环境
   ...
   // dataSet.print()
    ...
    var data2:DataStream[StationLog]=dataSet.map(line=>{
      val arr = line.split(",")
    ...
    //过滤掉那些通话成功的
    var data3=data2.filter(_.callType.equals("success"))
      .map(new CallMapFunction(format))
    data3.print()
    env.execute("StationLog_FunctionClassTransformation2")
    /***
